...
M.try_write_changes = function(confirm)
  ...
  if not vim.tbl_isempty(all_errors) then
    vim.notify("Error parsing fm buffers", vim.log.levels.ERROR)
  ...
  preview.show(actions, confirm, function(proceed)
    ...
    M.process_actions(
      ...
      vim.schedule_wrap(function(err)
        view.unlock_buffers()
        if err then
          vim.notify(string.format("[fm] Error applying actions: %s", err), vim.log.levels.ERROR)
          view.rerender_all_fm_buffers({ preserve_undo = false })
        else
          ...
          view.rerender_all_fm_buffers({ preserve_undo = M.trash })
        end
        mutation_in_progress = false
      end)
    )
