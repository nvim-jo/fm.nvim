...
---@alias fm.TextChunk string|string[]
...
M.toggle_float = function(dir)
  local util = require("fm.util")
  if util.is_fm_bufnr(0) and util.is_floating_win(0) then
...
---    close boolean Close the original fm buffer once selection is made
...
M.select = function(opts, callback)
  ...
  if util.is_floating_win() and opts.preview then
    return finish("fm preview doesn't work in a floating window")
  end
    ...
    get_edit_path(function(normalized_url)
      ...
      if opts.preview and not util.parse_url(filename) then
        ...
        if vim.fn.bufloaded(bufnr) == 0 then
          vim.bo[bufnr].bufhidden = "wipe"
          vim.b[bufnr].fm_preview_buffer = true
        end
      end
  ...
  return {
    {
      name = "FMDir",
      link = "Directory",
      desc = "Directories in an fm buffer",
    },
    {
      name = "FMDirIcon",
      link = "FMDir",
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    callback = function()
      ...
      if scheme and config.adapters[scheme] then
        require("fm.view").maybe_set_cursor()
        ...
        if not vim.w.fm_did_enter then
          require("fm.view").set_win_options()
